...
function StatusDot({ status }: { status: Track["status"] }) {
  if (status === "completed")
    return <CheckCircle2 size={12} className="shrink-0" style={{ color: "#d4af37" }} />;
  if (status === "in-progress")
    return <PlayCircle size={12} className="shrink-0 animate-pulse" style={{ color: "#d4af37" }} />;
  return <ChevronRight size={12} className="shrink-0" style={{ color: "rgba(255,255,255,0.3)" }} />;
}
