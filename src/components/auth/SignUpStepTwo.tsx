...
interface StepTwoProps {
  ...
  currentRole: string; setCurrentRole: (val: string) => void;
  stateRegion: string; setStateRegion: (val: string) => void;
  schoolLocation: string; setSchoolLocation: (val: string) => void;
  schoolType: string; setSchoolType: (val: string) => void;
  schoolName: string; setSchoolName: (val: string) => void;
...
export default function SignUpStepTwo({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const e = err as { statusCode?: number; status?: number; message?: string; error?: string; details?: { field?: string; message: string }[] };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="space-y-4">
        <AuthInput label="Full Name" id="reg-fullname" type="text" placeholder="Enter your full name" iconName="person" value={fullName} onChange={(e) => setFullName(e.target.value)} required />
        <AuthInput label="Phone Number" id="reg-phone" type="tel" placeholder="e.g. 08012345678" iconName="smartphone" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} required />

        {!isGoogleRoute && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label htmlFor="reg-pass" className="text-sm font-700 text-neutral-700 block font-body">Password</label>
              <div className="relative flex items-center">
                <Lock size={16} className="absolute left-4 text-neutral-500 pointer-events-none" />
                <input id="reg-pass" type={showPass ? "text" : "password"} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required
                ...
                <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 text-neutral-500 hover:text-neutral-700 focus:outline-none">
                  {showPass ? <EyeOff size={18} /> : <Eye size={18} />}
                </button>
              </div>
            </div>
            <div className="space-y-1.5">
              <label htmlFor="reg-confirm-pass" className="text-sm font-700 text-neutral-700 block font-body">Confirm Password</label>
              <div className="relative flex items-center">
                <ShieldCheck size={16} className="absolute left-4 text-neutral-500 pointer-events-none" />
                <input id="reg-confirm-pass" type={showConfirmPass ? "text" : "password"} placeholder="••••••••" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required
                  className="w-full bg-neutral-100 border border-neutral-300 text-neutral-800 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 text-sm font-500 font-body transition-all outline-none rounded-sm pl-11 pr-10 py-3" />
                <button type="button" onClick={() => setShowConfirmPass(!showConfirmPass)} className="absolute right-3 text-neutral-500 hover:text-neutral-700 focus:outline-none">
        ...
        <AuthInput label="School Name" id="reg-school" type="text" placeholder="Enter your school/institution name" iconName="school" value={schoolName} onChange={(e) => setSchoolName(e.target.value)} required />
      </div>
